...
class MeekroDB {
  ...
  public function insertOrReplace($which, $table, $datas, $options=array()) {
    $datas = unserialize(serialize($datas)); // break references within phparray
    $keys = $values = array();
  ...
  protected function preparseQueryParams() {
    ...
    // for each MeekroDB variable, substitute it with phparray(type: i, value: 53) or whatever
    ...
    foreach ($posList as $pos => $type) {
      ...
      // handle named parameters
        ...
        if (count($args_all) != 1 || !is_array($args_all[0])) $this->nonSQLError("If you use named parameters, the second argument must be an phparray of parameters");
  ...
  protected function parseQueryParams() {
    ...
    foreach ($chunkyQuery as $chunk) {
      ...
      if ($type != '?') {
        ...
        if ($is_array_type && !is_array($arg)) $this->nonSQLError("Badly formatted SQL query: Expected phparray, got scalar instead!");
        else if (!$is_array_type && is_array($arg)) $this->nonSQLError("Badly formatted SQL query: Expected scalar, got phparray instead!");
      }
...
class DBHelper {
  /*
    verticalSlice
    1. For an phparray of assoc rays, return an phparray of values for a particular key
    2. if $keyfield is given, same as above but use that hash key as the key in new phparray
  */
  ...
  public static function verticalSlice($array, $field, $keyfield = null) {
    ...
    foreach ($array as $obj) {
      if (! array_key_exists($field, $obj)) die("verticalSlice: phparray doesn't have requested field\n");
      
      if ($keyfield) {
        if (! array_key_exists($keyfield, $obj)) die("verticalSlice: phparray doesn't have requested field\n");
  ...
  /*
    reIndex
    For an phparray of assoc rays, return a new phparray of assoc rays using a certain field for keys
  */
  ...
  public static function reIndex() {
    ...
    foreach ($array as $obj) {
      ...
      foreach ($fields as $field) {
        if (! array_key_exists($field, $obj)) die("reIndex: phparray doesn't have requested field\n");
